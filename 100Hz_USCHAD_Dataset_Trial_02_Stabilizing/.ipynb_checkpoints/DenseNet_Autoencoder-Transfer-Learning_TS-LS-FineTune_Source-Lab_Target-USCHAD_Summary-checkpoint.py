...
sample_cases = np.sort(np.concatenate([np.arange(23,35,1),np.arange(2,7,1),np.arange(19,23,1)]))
test_set_size = free_liv_main_test_size

save_models_var = 'DenseNet-Autoencoder-Transfer-Learning-TS-LS-Fine-Tune_Source-Lab_Target-USCHAD' 
...
if os.path.exists(code_path+'/Unsupervised_Transfer_Learning_TS-LS-Fine-Tune_Summary_Window-'+str(window_size)+'.pckl'):
    f = open(code_path+'/Unsupervised_Transfer_Learning_TS-LS-Fine-Tune_Summary_Window-'+str(window_size)+'.pckl','rb')
    temp = pickle.load(f)
    f.close()
...
for free_liv_train_size_index in amount_of_data_indices:
    ...
    for seed_value_index in seed_indices:
        ...
        for trial_index in trial_indices:
            ...
            new_model = timeseries_classifier_model_02([x_train_ls.shape[1],x_train_ls.shape[2]], 8, [4,4,4,4])
            for new_layer, layer in zip(new_model.layers[2:], pretrained_model.layers[2:]):
                new_layer.set_weights(layer.get_weights())
            #pretrained_model.summary()


            layer_name = 'global_average_pooling1d_1'
            new_model = models.Model(inputs=new_model.input, outputs=new_model.get_layer(layer_name).output)

            last_layer= models.Sequential()
            last_layer.add(layers.Dense(y_train.shape[1],activation='softmax'))

            classifier_model = models.Model(inputs=new_model.input, outputs=last_layer(new_model.output))
            ...
            y_test_predict = classifier_model.predict(x_test_ls)
            ...
            f = open(code_path+'/Unsupervised_Transfer_Learning_TS-LS-Fine-Tune_Summary_Window-'+str(window_size)+'.pckl','wb')
