...
save_models_var = 'DenseNet-KA-Dual_T-'+str(T)+'_Source-Lab_Target-USCHAD' 
...
amount_of_data_indices = np.arange(5,6,dtype=int)

if os.path.exists(code_path+'/KA_T-'+str(T)+'_Summary_Window-'+str(window_size)+'.pckl'):
    f = open(code_path+'/KA_T-'+str(T)+'_Summary_Window-'+str(window_size)+'.pckl','rb')
    temp = pickle.load(f)
    f.close()
...
for free_liv_train_size_index in amount_of_data_indices:
    ...
    for seed_value_index in seed_indices:
        ...
        ## Pre-trained model
        teacher_model_path = code_path + '/DenseNet-Supervised_TrainLab_TestLab' + '_WindowSize-' + str(window_size) + '/TrainSize-90_TestSize-10/Seed-'+str(2)+'/Trial-1_Model-250-epochs.h5'
        ...
        for trial_index in trial_indices:
            ...
            student_model.load_weights(classifier_models_path + '/Seed-'+str(seed_value) + '/KA-Trial-' + str(trial) + '_Model-310-epochs.h5')

            print('Train data shape: ',x_train.shape)
            print('Test data shape: ',x_test.shape)
            print('Train label shape: ',y_train.shape)
            print('Test label shape: ',y_test.shape)
            _,y_test_predict = student_model.predict(x_test)
            ...
            f = open(code_path+'/KA_T-'+str(T)+'_Summary_Window-'+str(window_size)+'.pckl','wb')
