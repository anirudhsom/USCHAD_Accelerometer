...
save_models_var = 'DenseNet-Transfer-Learning-Fine-Tune_Source-Lab_Target-USCHAD' 
...
if os.path.exists(code_path+'/Transfer_Learning_Fine-Tune_Summary_Window-'+str(window_size)+'.pckl'):
    f = open(code_path+'/Transfer_Learning_Fine-Tune_Summary_Window-'+str(window_size)+'.pckl','rb')
    temp = pickle.load(f)
    f.close()
...
for free_liv_train_size_index in amount_of_data_indices:
    ...
    for seed_value_index in seed_indices:
        ...
        for trial_index in trial_indices:
            ...
            new_model = models.Model(inputs=pretrained_model.input, outputs=pretrained_model.get_layer(layer_name).output)
            ...
            f = open(code_path+'/Transfer_Learning_Fine-Tune_Summary_Window-'+str(window_size)+'.pckl','wb')
