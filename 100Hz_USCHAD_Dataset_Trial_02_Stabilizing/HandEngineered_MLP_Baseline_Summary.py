...
if os.path.exists(code_path+'/Supervised-MLP-Baseline_Learning_Summary_Window-'+str(window_size)+'.pckl'):
    f = open(code_path+'/Supervised-MLP-Baseline_Learning_Summary_Window-'+str(window_size)+'.pckl','rb')
    temp = pickle.load(f)
    f.close()
...
for free_liv_train_size_index in amount_of_data_indices:
    ...
    for seed_value_index in seed_indices:
        ...
        for trial_index in trial_indices:
            ...
            model.load_weights(classifier_models_path + '/Seed-'+str(seed_value) + '/Trial-' + str(trial) + '_Model-250-epochs.h5')
            
            y_test_predict = model.predict(x_test_feat)
            ...
            f = open(code_path+'/Supervised-MLP-Baseline_Learning_Summary_Window-'+str(window_size)+'.pckl','wb')
